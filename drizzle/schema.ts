...
export const projects = mysqlTable("projects", {
	...
	status: mysqlEnum(['inquiry','calculation','offer','order','production','shipping','completed','cancelled','rejected']).default('inquiry').notNull(),
	customerId: int("customer_id"),
	supplierId: int("supplier_id"),
	...
	rejectionReason: mysqlEnum("rejection_reason", ['preis','timing','wettbewerber','kein_feedback','sonstiges']),
	rejectionNote: text("rejection_note"),
	reactivatedAt: timestamp("reactivated_at", { mode: 'string' }),
	createdAt: timestamp({ mode: 'string' }).default('CURRENT_TIMESTAMP').notNull(),
	updatedAt: timestamp({ mode: 'string' }).defaultNow().onUpdateNow().notNull(),
});
